    ...
    <>
      ...
      <label className="input input-lg w-full">
        ...
        <button type="button" onClick={() => setShowPass((prev) => !prev)}>
          {showPass ? (
            <LuEyeClosed className="opacity-50" />
          ) : (
            <LuEye className="opacity-50" />
          )}
        </button>
      </label>
      ...
      {!login && password.length > 0 && (
        <>
          ...
          <ul className="text-sm mt-2">
            ...
            <li
              className={`flex items-center gap-2 ${
                rules.lowercase ? "text-green-600" : "text-red-600"
              }`}
            >
              {rules.lowercase ? <LuCheck /> : <LuX />} Contains lowercase
              letter
            </li>
            <li
              className={`flex items-center gap-2 ${
                rules.uppercase ? "text-green-600" : "text-red-600"
              }`}
            >
              {rules.uppercase ? <LuCheck /> : <LuX />} Contains uppercase
              letter
            </li>
            <li
              className={`flex items-center gap-2 ${
                rules.number ? "text-green-600" : "text-red-600"
              }`}
            >
