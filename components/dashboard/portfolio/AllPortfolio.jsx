    ...
    <>
      <div className="grid grid-cols-5 rounded-xl gap-5">
        {isLoading ? (
          ...
          <p className="text-center">No Portfolio has been added yet</p>
        ) : (
          portfolios.portfolios.map((port) => (
            <PortfolioCard client={false} key={port._id} portfolio={port} />
          ))
        )}
      </div>
      {isLoading ? (
