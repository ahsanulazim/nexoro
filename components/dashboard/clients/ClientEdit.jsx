...
export default function ClientEdit({ ref, client }) {
    ...
    const mutation = useMutation({
        ...
        onSettled: () => {
            // reset(); // Redundant with 'values' prop
        }
    });

    const handleClient = (data) => {
        mutation.mutate({ id: client._id, formData: data });
            ...
            <div className="modal-box">
                <form className="fieldset" onSubmit={handleSubmit(handleClient)}>
                    ...
                    <div className="modal-action">
                        ...
                        <button type="submit" className={`btn btn-primary ${mutation.isPending || !isDirty ? "" : "btn-nexoro-primary"}`} disabled={mutation.isPending || !isDirty}>
                            {mutation.isPending && <span className="loading loading-spinner"></span>} Update Client </button>
                    </div>
                </form>
            </div>
        </dialog>
