            ...
            {!isLoading && <div className="flex justify-between items-center">
                <div className="join">
                    ...
                    {[...Array(data.totalPages)].map((_, i) => {
                        const pageNum = i + 1;
                        return (
                            <button key={pageNum} className={`join-item btn ${page === pageNum ? "btn-active btn-success" : ""}`} onClick={() => goToPage(pageNum)}>{pageNum}</button>
                        );
                    })}
                    <button className="join-item btn" disabled={!data?.hasNext} onClick={() => goToPage(page + 1)}>»</button>
                </div>
                <p className="text-sm hidden md:block">Showing {data.start} to {data.end} of {data.totalBlogs} Blogs ({data.totalPages} Pages)</p>
            </div>}
        </>
    )
}
