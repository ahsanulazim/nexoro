...
const BlogCard = ({ blog }) => {
  return (
    <div className="card bg-base-300 shadow-sm overflow-hidden">
      <Link href={`/articles/${blog.slug}`} className="w-full">
        <img
      ...
      <div className="card-body justify-between">
        <div>
          ...
          <Link
            ...
            className="card-body p-0 mt-1 line-clamp-3"
          >
            <h2 className="card-title">{blog.title}</h2>
          </Link>
          <p className="flex gap-2 text-xs">
            <span className="opacity-50">
              {new Date(blog.added).toLocaleString("en-BD", {
                timeZone: "Asia/Dhaka",
                dateStyle: "medium",
              })}
            </span>
            •<span className="link link-hover link-info">{blog.author}</span>
          </p>
          <Link href={`/articles/${blog.slug}`} className="card-body p-0 mt-3">
